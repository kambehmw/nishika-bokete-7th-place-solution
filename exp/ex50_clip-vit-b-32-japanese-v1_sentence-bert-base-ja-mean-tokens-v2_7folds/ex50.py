...
from torchvision.models import ResNet152_Weights, resnet152, ViT_B_16_Weights, vit_b_16, Swin_B_Weights, swin_b
...
class CFG:
    vision_model_weight="sonoisa/clip-vit-b-32-japanese-v1"
    text_model="sonoisa/sentence-bert-base-ja-mean-tokens-v2"
    max_len=48
    seed=42
    n_fold=7
    num_labels=2
...
class ImageEncoder(nn.Module):
    def __init__(self, pretrained_weight):
        super().__init__()
        self.model = CLIPVisionModel.from_pretrained(CFG.vision_model_weight)
        config = self.model.config
        self.head = nn.Linear(config.hidden_size, 2048)
...
class BoketeTextImageDataset(Dataset):
    def __init__(self, df, tokenizer, max_seq_len:int, processor):
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        image = self.processor(images=read_jpg(row["img_path"]), return_tensors="pt")["pixel_values"][0]
...
repo_dir = snapshot_download(CFG.vision_model_weight)
print(repo_dir)
# processor = CLIPProcessor.from_pretrained(os.path.join(repo_dir, "vision_model"))
feature_extractor = CLIPFeatureExtractor.from_pretrained(os.path.join(repo_dir, "vision_model"))
# # Data Split
...
def train_loop(folds, fold):
    ...
    trn_ds = BoketeTextImageDataset(train_folds, tokenizer, CFG.max_len, feature_extractor)
    val_ds = BoketeTextImageDataset(valid_folds, tokenizer, CFG.max_len, feature_extractor)
...
test_ds = BoketeTextImageDataset(test_df, tokenizer, CFG.max_len, feature_extractor)
