...
import transformers
from modeling_mmbt import MMBTForClassification
...
class CFG:
    vision_model_weight=Swin_B_Weights.IMAGENET1K_V1
    ...
    fp16=True
    debug=False
    pseudo_df="../ex57_inference_pseudo_labeling/pseudo_fold{}.csv"
...
train_df[[f"pred_{i}" for i in range(CFG.num_labels)]] = 0.0
train_df.loc[train_df["is_laugh"] == 0, "pred_0"] = 1.0
train_df.loc[train_df["is_laugh"] == 1, "pred_1"] = 1.0
...
class ImageEncoder(nn.Module):
    def __init__(self, pretrained_weight):
        super().__init__()
        self.model = swin_b(weights=pretrained_weight)
        self.head = nn.Linear(self.model.head.in_features, 2048)
...
class BoketeTextImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "sentence": sentence,
            "image": image,
            "label": torch.tensor(row[["pred_0", "pred_1"]], dtype=torch.float32),
        }
...
def train_loop(folds, fold):
    ...
    if CFG.pseudo_df:
        print("Train: ", train_folds.shape)
        pseudo_df = pd.read_csv(CFG.pseudo_df.format(fold))
        train_folds = pd.concat([train_folds, pseudo_df])
        print("Train + Pseudo shape: ", train_folds.shape)
    ...
    val_preds = torch.sigmoid(torch.FloatTensor(val_preds)).numpy()
    
    valid_folds[[f"pred_{i}" for i in range(CFG.num_labels)]] = val_preds

    # sanity check
    score = log_loss(valid_labels, val_preds)
